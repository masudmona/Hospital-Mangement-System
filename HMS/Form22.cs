...
namespace HMS
{
    public partial class Form22_new_acc : Form
    {
        ...
        private void Form22_new_acc_Load(object sender, EventArgs e)
        {
        ...
        private void button_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    errorUsername.SetError(textBox_name, "Enter A Username");
                    errorPassword.SetError(textBox_pass, null);
                }
                else if (textBox_pass.Text == "")
                {
                    errorPassword.SetError(textBox_pass, "Enter A Valid Password");
                    errorUsername.SetError(textBox_name, null);
                }
                else
                {
                    errorPassword.SetError(textBox_pass, null);
                    ...
                    using (SqlConnection conString = new SqlConnection(@"Data Source=S1S1S1-PC\SQLEXPRESS;Initial Catalog=HMS;Integrated Security=True"))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("select count(*) from [Admin] where Username = @Username", conString))
                        {
                        ...
                        if (exists)
                            errorPassword.SetError(textBox_name, "This username has been using by another user.");
                        else
                        {
                            // does not exists, so, persist the user
                            using (SqlCommand cmd = new SqlCommand("INSERT INTO [Admin] values (@Username, @Password)", conString))
                            {
                                cmd.Parameters.AddWithValue("Username", textBox_name.Text);
