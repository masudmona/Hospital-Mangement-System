...
namespace HMS
{
    public partial class Form12_Doc_schedule : Form
    {
        ...
        private void Form12_Doc_schedule_Load(object sender, EventArgs e)
        {
            ...
            // TODO: This line of code loads data into the 'hMSDataSetNew.Doctor_schedule' table. You can move, or remove it, as needed.
            this.doctor_scheduleTableAdapter.Fill(this.hMSDataSetNew.Doctor_schedule);
            display_data();
            doctor_scheduleDataGridView.ColumnHeadersDefaultCellStyle.Font = new Font("Microsoft Sans Serif", 11, FontStyle.Bold);
            doctor_scheduleDataGridView.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.TopCenter;
            doctor_scheduleDataGridView.RowsDefaultCellStyle.BackColor = Color.FromArgb(221, 140, 111);
            doctor_scheduleDataGridView.DefaultCellStyle.Font = new Font("Microsoft Sans Serif", 11, FontStyle.Regular);
        ...
        {
            string query = @"INSERT INTO Doctor_schedule(D_ID, Department, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Sturday)
                            VALUES('" + comboBox_DID.Text + "','" + comboBox_dept.Text + "','" + textBox_sun.Text + "','" + textBox_mon.Text + "','" + textBox_tue.Text + "', '" + textBox_wed.Text + "', '" + textBox_thu.Text + "', '" + textBox_fri.Text + "', '" + textBox_sat.Text + "') ";
            try
            {
                conString.Open();
        ...
        {
            sda = new SqlDataAdapter(@"select * from Doctor_schedule", conString); //sda=cmd
            dt = new DataTable();
            sda.Fill(dt);
            doctor_scheduleDataGridView.DataSource = dt;
        ...
        private void textBox_sat_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            sda = new SqlDataAdapter(@"select * from  Doctor_schedule  WHERE  D_ID like '" + textBox_search.Text + "%' or Sunday like '%" + textBox_search.Text + "%' or Monday like '" + textBox_search.Text + "%' or Tuesday like '%" + textBox_search.Text + "%' or Wednesday like '%" + textBox_search.Text + "%' or Thursday like '%" + textBox_search.Text + "%' or Friday like '%" + textBox_search.Text + "%' or Sturday like '%" + textBox_search.Text + "%'", conString);
            dt = new DataTable();
            sda.Fill(dt);
            doctor_scheduleDataGridView.DataSource = dt;
            doctor_scheduleDataGridView.RowsDefaultCellStyle.BackColor = Color.Blue;
            doctor_scheduleDataGridView.RowsDefaultCellStyle.ForeColor = System.Drawing.Color.White; 
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    scb = new SqlCommandBuilder(sda);
                    doctor_scheduleDataGridView.Rows.RemoveAt(doctor_scheduleDataGridView.CurrentRow.Index);
                    sda.Update(dt);
        ...
        private void comboBox_DID_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd1 = new SqlCommand("select Department from Doctor where D_ID like '" + comboBox_DID.Text + "' ", conString))
                    {
                        conString.Open();
